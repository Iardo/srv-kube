...
- A `llama-server` request with an image attached can fail with `failed to launch ffprobe` / `failed to decode webp buffer`, this only affects `.webp` images, `stb_image` (llama.cpp's built-in decoder) doesn't support that format and falls back to `ffmpeg`/`ffprobe`, which don't ship with Windows by default, fix with `winget install --id Gyan.FFmpeg --source winget` then a **new** terminal
